sap.ui.define([
	...
	return t.extend("MM.mat_consumption.controller.Home", {
			...
			oModelGOODSMVNT.read("/YY1_GOODS_MVMT", {
				...
				success: function (data, err) {
					...
					if (s !== 0) {
						...
						for (var r = 0; r < s; r++) {
							if (o[r].Material !== "" && o[r].Plant !== "" && (o[r].GoodsMovementType === "261" || o[r].GoodsMovementType === "262" || o[
		...
		_buildFinalData: function (oConData) {
					...
					if (e.MatStock[r].Material === e.StdPrice[s].Material && e.MatStock[r].Plant === e.StdPrice[s].ValuationArea) {
						...
						for (var p = 0; p < e.GOODS_MVMT_CUM.length; p++) {
							if (e.MatStock[r].Material === e.GOODS_MVMT_CUM[p].Material && e.MatStock[r].Plant === e.GOODS_MVMT_CUM[p].Plant) {
								...
								a.Consumption262 = 0;
								if ((e.GOODS_MVMT_CUM[p].Plant === "US01" || e.GOODS_MVMT_CUM[p].Plant === "US02" || e.GOODS_MVMT_CUM[p].Plant === "US03") &&
									e.GOODS_MVMT_CUM[p].GoodsMovementType === "261" || e.GOODS_MVMT_CUM[p].GoodsMovementType === "262"
								) {

									for (var x = 0; x < e.GOODS_MVMT_CUM.length; x++) {
										if (e.GOODS_MVMT_CUM[x].Material === e.GOODS_MVMT_CUM[p].Material &&
											e.GOODS_MVMT_CUM[x].Plant === e.GOODS_MVMT_CUM[p].Plant &&
											(e.GOODS_MVMT_CUM[x].GoodsMovementType === "261")
										) {

											a.Consumption261 = parseFloat(e.GOODS_MVMT_CUM[x].TotalGdsMvtQtyInBaseUnit);
										}
										if (e.GOODS_MVMT_CUM[x].Material === e.GOODS_MVMT_CUM[p].Material &&
											e.GOODS_MVMT_CUM[x].Plant === e.GOODS_MVMT_CUM[p].Plant &&
											(e.GOODS_MVMT_CUM[x].GoodsMovementType === "262")
										) {

											a.Consumption262 = parseFloat(e.GOODS_MVMT_CUM[x].TotalGdsMvtQtyInBaseUnit);
										}
									}
						...
						if (a.Consumption261 === 'undefined') {
							a.Consumption261 = 0;
						}
						if (a.Consumption262 === 'undefined') {
							a.Consumption262 = 0;
						}

						a.Consumption = parseFloat(a.Consumption261) + parseFloat(a.Consumption262);
						...
						if (a.CrossPlantStatus === "05") {
							a.ExcessQOH = parseFloat(a.On_Hand_Qty).toFixed(2);
						}
						if (a.CrossPlantStatus === "03") {
							var m = parseFloat(a.MonthUsage24) - parseFloat(a.On_Hand_Qty);
							if (m > 0) {
								a.ExcessQOH = parseFloat(m).toFixed(2);
							} else {
								a.ExcessQOH = "0";
							}
						}
						...
						if (a.NEW_OBS_EXC_or_GOOD === "OBS") {
							...
							a.Obsolete_OH_FIFO = parseFloat(a.Obsolete_OH_FIFO).toFixed(2);
						} else {
							a.Obsolete_OH_FIFO = "0";
						}

						a.E_O_OH_FIFO = a.Excess_OH_FIFO + a.Obsolete_OH_FIFO;
						a.E_O_OH_FIFO = parseFloat(a.E_O_OH_FIFO).toFixed(2);
			...
			// O = {};
			// O
			// 	.columnName = "Months_Usage_Life_in_months";
			// M.push(O);
			// O = {};
			...
			// f.setData(S.BotList, "S");
			// // var R = new sap.ui.model.json.JSONModel;
			// this.getView().setModel(f, "S");
