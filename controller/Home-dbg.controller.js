sap.ui.define([
	...
	return t.extend("MM.mat_consumption.controller.Home", {
					...
					if (s !== 0) {
						...
						for (var r = 0; r < s; r++) {
							...
							if (o[r].Material !== "" && o[r].Plant !== "") {
								...
								// if (o[r].CreationDate === undefined) {
								// 	o[r].CreationDate = '0000000000';
								// }
		...
		_getGoods_Mvnt: function (t, resolve, reject) {
			...
			var M = new sap.ui.model.Filter("DocumentDate", sap.ui.model.FilterOperator.LE, todate.toISOString());
			var O = new sap.ui.model.Filter({
				filters: [M],
				and: true
			});
			var S = new sap.ui.model.Filter("DocumentDate", sap.ui.model.FilterOperator.GE, fromdate.toISOString());
		...
		_getGoods_MvntQty: function (t, resolve, reject) {
			...
			// var yyyy = idDatePickerVal.slice(6, 10) - 2;
			// var mm = idDatePickerVal.slice(0, 2);
			// var dd = idDatePickerVal.slice(3, 5);
			// var fromDate = yyyy + "-" + mm + "-" + dd;
			// var fromDate = new Date(fromDate);
		...
		_getSL099_QUANTITY: function (t, resolve, reject) {
			// https://my301187-api.s4hana.ondemand.com/sap/opu/odata/sap/YY1_MATERIAL_IN_STOCK_CDS/YY1_Material_in_Stock?$filter=Plant eq 'US01' and Material eq 'M301009025' and MatlDocLatestPostgDate le datetime'2020-08-11T00:00:00'&$select=Material,Plant,MatlWrhsStkQtyInMatlBaseUnit
			var oModelMATSTOCK = this.getOwnerComponent().getModel("MATSTOCK");
			var aSL099_QUANTITY = {
				SL099_QUANTITY: []
			};
		...
		_getQOH: function (t, resolve, reject) {
			...
			if (this !== undefined) {
				...
				// var S = new sap.ui.model.Filter("MatlDocLatestPostgDate", sap.ui.model.FilterOperator.GE, fromDate.toISOString());
				// var v = new sap.ui.model.Filter({
				// 	filters: [S],
				// 	and: true
				// });
				...
				oModelMATSTOCKonHnd.read("/YY1_Material_in_Stock", {
					...
					success: function (data, err) {
						...
						// MatlDocLatestPostgDate data["results"][0].MatlDocLatestPostgDate.toISOString()
						// 
						var o = data.results;
						o.sort();
						var s = o.length;
		...
		// _getQOHL: function (t, resolve, reject) {
		// 	

		// 	if (this !== undefined) {
		// 		var idFieldPlantVal = this.getView().byId("idFieldPlant").getValue().toUpperCase();
		// 		var r = new sap.ui.model.Filter("Material", sap.ui.model.FilterOperator.EQ, idFieldPlantVal);
		// 		var i = new sap.ui.model.Filter({
		// 			filters: [r],
		// 			and: true
		// 		});
		...
		// 		var M = new sap.ui.model.Filter("P_KeyDate", sap.ui.model.FilterOperator.LE, toDate.toISOString());
		// 		var O = new sap.ui.model.Filter({
		// 			filters: [M],
		// 			and: true
		// 		});
		...
		// 		};
		// 		
		// 		// var val = toDate.toISOString();
		// 		 var val = this.getView().byId("idDatePicker").getValue().toUpperCase();
		// 		var oModelMATSTOCKonHnd = this.getOwnerComponent().getModel("MATONHANDQTY");
		// 		oModelMATSTOCKonHnd.read("/YY1_MatOnHandQty(P_KeyDate='" + val + "')", {
		// 			// urlParameters: {
		// 			// 	$select: "Product, Plant, MatlWrhsStkQtyInMatlBaseUnit"
		// 			// },
		// 			// filters: [f],
		// 			success: function (data, err) {
		// 				
			...
			for (var s = 0; s < e.StdPrice.length; s++) {
				for (var r = 0; r < e.MatStock.length; r++) {
					if (e.MatStock[r].Material === e.StdPrice[s].Material && e.MatStock[r].Plant === e.StdPrice[s].ValuationArea) {
						...
						// var d = l.slice(0, 4); //yyyy
						// var u = l.slice(5, 7); //mm
						...
						if (e.MatStock[r].CreationDate === null
							// || e.MatStock[r].CreationDate.getFullYear() === 0000 
						) {

						} else {
							var _ = e.MatStock[r].CreationDate.getFullYear();
						}
						// 	if (e.MatStock[r].CreationDate.getFullYear() === 0000 
						// ) {
						// 	
						// }
						if (e.MatStock[r].CreationDate === null) {
			...
			if (t !== '' && DatePicker !== '') {
				...
				promiseMatStock.then(function (mStock) {
					...
					promiseStd_Price = new Promise(function (resolve, reject) {
						...
						// that._getQOHL(t, resolve, reject);
